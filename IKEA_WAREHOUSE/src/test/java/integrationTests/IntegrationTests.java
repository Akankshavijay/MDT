...
public class IntegrationTests {
	...
	private void waitUntil(Supplier<Boolean> condition, long timeoutMillis, String failMessage)
	...
	public void chargingMngEndToEndTest() throws Exception {
		...
		waitUntil(
				() -> robot.getBattery() > 10 && station.getCurrentRobot() == null
						&& station.getStatus() == ChargingStation.Status.READY,
				15_000L, "Robot should be charged and station should be READY again");
	...
	public void robotMngAndChargingMngEndToEndTest() throws Exception {
		...
		robotManager.enqueueRobotTask(RobotTask.moveTo(5, 5), dummyTask);

		waitUntil(
				() -> robot.getBattery() > 10 && robot.getStatus() == Robot.Status.READY
						&& station.getCurrentRobot() == null && station.getStatus() == ChargingStation.Status.READY,
				20_000L, "Robot should have been charged via RobotManager + ChargingManager");
	...
	public void taskMngStoreTaskEndToEndTest() throws Exception {
		...
		RobotManager robotManager = new RobotManager("RobotManager", logger);
		Robot robot = new Robot("R1", 0, 0, 100, "Robot", logger);
		robotManager.addRobot(robot);
		
		ChargingManager chargingManager = new ChargingManager("ChargingManager", logger);
		ChargingStation chargingStation = new ChargingStation("S1", "ChargingStation", logger);
		chargingManager.addStation(chargingStation);
		robotManager.setChargingManager(chargingManager);
		...
		waitUntil(() -> storeTask.getState() == TaskState.DONE && robot.getStatus() == Status.READY, 10_000L,
	...
	public void taskMngRetriveTaskEndToEndTest() throws Exception {
		...
		waitUntil(() -> retrieveTask.getState() == TaskState.DONE, 10_000L, "RETRIEVE task should reach DONE state");
